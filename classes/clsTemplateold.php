...

		{
...

			{
...

				{
					...
					echo $fieldValue	= $this->getShareTransfer();
					//die;
  ...
  //                       <th style="border-right: 1px solid;">POSITION HELD</th>
	...
	function getsharebalance($consumeruser_id,$date,$sharesorgbuy ,$cert_no_alloted)
	{
		...
		// 		}
				...
				echo $ShresBalance = $sharesbuy+$sharesorgbuy-$sharessold;
	...
	function getShareTransfer()
	{
...

		{
			...
			while($details=mysqli_fetch_object($query))
			{
				...
				echo $displaydt = $this->checkDateofTransfer($details->consumeruser_id,$details->consumersharecertno);
			...
			echo $select= "Select a.*, b.consumeraddress1,b.consumercity, b.consumerstate_id,b.consumerzipcode, b.consumersharecertno, b.consumeruser_id  FROM  tbl_sharetransfer_data a,  tbl_consumeruser b WHERE from_userid >= 0 and to_userid = b.consumeruser_id and consumer_id = '".$this->consumer_id."'  order by date";
			...
			if(mysqli_num_rows($tt_query)>0)
			{
				...
				{
					echo $consumersharecertno = $details_t->consumersharecertno;
					echo "<br>";
					echo $details_t->cert_no_issued_to;
					echo "<br>";
					echo $details_t->to_userid;
					echo "<br>";
					echo $consumeruser_id = $details_t->consumeruser_id;
					echo "<br>";
					echo $details_t->from_userid;
					echo "<br>";
					//$details_t->cert_no_issued_to != $consumersharecertno and

					if ( $details_t->to_userid == $consumeruser_id and $details_t->from_userid >= 0 and !(in_array($details_t->cert_no_issued_from, $intial_cert)))
					{
					...
					$certno_cancelled = $details_t->cert_no_issued_from;
					while(true)
					{
						$select="Select *   FROM  tbl_sharetransfer_data WHERE from_userid = '".$consumeruser_id."' and cert_no_cancelled = '".$certno_cancelled."'";
			...
			if(mysqli_num_rows($tt_query)>0)
			{
				while($details_t=mysqli_fetch_object($tt_query))
				{
					...
					if ( $details_t->to_userid == $consumeruser_id and $details_t->from_userid >= 0 and !(in_array($details_t->cert_no_issued_to, $intial_cert)))
					{
		...
		if(mysqli_num_rows($query)>0)
		{
			...
			$detail	= $detail.'
					...
					<tr style="border-bottom: 1px solid;text-align: center;">
                        ...
                        <th style="border-right: 1px solid;">DATE WHEN CEASED TO A SHAREHOLDER</th>
...

			{
				...
				if (!empty($firstdt))
				{
					if (strtotime($firstdt) != strtotime($incorpdt))
					{
						echo $incorpdate =  date("jS \of F Y",strtotime($firstdt));
					}
				}
...

	{
		...
		$Shareholdersignature = '__________________________'.'<br>'.$row->fieldValue;
...

	{
		...
		echo $sqlQry="SELECT * FROM tbl_consumeruser WHERE consumer_id ='".$this->consumer_id."' and (consumerisofficer=1 or  consumerisofficer=2) and  (consumeruser_id not in (select consumeruser_id  from tbl_members_servicerec where member_designation ='officer') or consumeruser_id in (select consumeruser_id  from tbl_members_servicerec where consumeruser_id = tbl_consumeruser.consumeruser_id and member_designation ='officer' and (member_doj = '".$incorpdate."' or (member_doj IS NULL and member_dol != ''))))";
...

	{
		...
		echo $sqlQry="SELECT  *  FROM tbl_consumeruser WHERE consumer_id ='".$this->consumer_id."' and consumerisshareholder=1 ";
		...
		if(mysqli_num_rows($res)>0)
		{
			...
			{
				$flag =0;
				echo $sqlQry="SELECT * FROM tbl_sharetransfer_data WHERE cert_no_issued_to  = '".$row->consumersharecertno."' and to_userid ='".$row->consumeruser_id."'";
...

	{
		...
		echo $sqlQry="SELECT concat(UCASE(consumerfname),' ',UCASE(consumermname), ' ' ,UCASE(consumerlname))  fieldValue FROM tbl_consumeruser WHERE consumer_id ='".$this->consumer_id."' and consumerisdirector=1";
...

	{
		...
		echo $sqlQry="SELECT concat(UCASE(consumerfname),' ',UCASE(consumermname), ' ' ,UCASE(consumerlname))  fieldValue FROM tbl_consumeruser WHERE consumer_id ='".$this->consumer_id."' and (consumerisofficer=1 or  consumerisofficer=2) and  (consumeruser_id not in (select consumeruser_id  from tbl_members_servicerec where member_designation ='officer') or consumeruser_id in (select consumeruser_id  from tbl_members_servicerec where consumeruser_id = tbl_consumeruser.consumeruser_id and member_designation ='officer' and (member_doj = '".$incorpdate."' or (member_doj IS NULL and member_dol != ''))))";
	...
	{
		echo $sqlQry="select updatedDate from tbl_consumermaster where consumer_id='".$this->consumer_id."'";
		...
		echo $sqlQry="select consumersharecertno from tbl_consumeruser where consumeruser_id='".$this->consumeruser_id."'";

		$res=mysqli_query($this->dbconnection,$sqlQry);
		$row=mysqli_fetch_object($res);
		$cert_no 	=	$row->consumersharecertno;	 

		echo $sqlQry="SELECT date FROM tbl_sharetransfer_data WHERE cert_no_issued_to  = '".$cert_no."' and to_userid ='".$this->consumeruser_id."' and from_userid = 0";
...

	{
		...
		echo $sqlQry="SELECT consumeruser_id, consumersharecertno, concat(UCASE(consumerfname),' ',UCASE(consumermname), ' ' ,UCASE(consumerlname)) fieldValue FROM tbl_consumeruser WHERE consumer_id ='".$this->consumer_id."' and consumerisshareholder=1 and consumershareright='Voting' ";
...

		{
...

			{
				...
				if($shareholdersignature=='')

					$shareholdersignature = '__________________________'.'<br>'.$row->fieldValue;

				else

					$shareholdersignature = $shareholdersignature .'<br>'.'<br>'.'__________________________'.'<br>'. $row->fieldValue;
	...
	function getShareholderDetailsType_transfer()
	{
		...
		{
			echo $value =  $this->getcancelled_info('4');
	...
	function getcancelled_info($field_type)
	{
		...
		while($row_cert=mysqli_fetch_object($res_cert))
		{
			if ($this->consumeruser_id == $row_cert->from_userid)
			{
				...
				{
					echo $sqlQry="SELECT consumersharecertno,consumernoofshares,concat( ' \" ', consumershareclass, ' \" ', consumersharetype, ' ', consumershareright )  fieldValue   FROM tbl_consumeruser WHERE consumeruser_id ='".$this->consumeruser_id."' and consumersharecertno = '".$cert_no."'";
					...
					{
						echo $sqlQry="SELECT *,concat( ' \" ', consumershareclass, ' \" ', consumersharetype, ' ', consumershareright )  fieldValue FROM tbl_sharetransfer_data WHERE (cert_no_issued_from = '".$cert_no."' and from_userid ='".$this->consumeruser_id."') or (cert_no_issued_to = '".$cert_no."' and to_userid ='".$this->consumeruser_id."') ";
	...
	function getShareholder_transdate()
	{
		$date = '';
		$sqlQry="select DATE_FORMAT(updatedDate,'%D day of %M, %Y') fieldValue from tbl_consumermaster where consumer_id='".$this->consumer_id."'";
		$res=mysqli_query($this->dbconnection,$sqlQry);

...
define('VALIDITY_TIME', 3600);
	Class User
	{
     ...
     /*---------------------------------token------------------------------ */
		...
		// {
		// 	$sqlQry="select * from tbl_user  where useremail='".$useremail."' ";
		// 	$res=mysqli_query($this->dbconnection,$sqlQry);
		// 	$row=mysqli_fetch_object($res);
		// 	return $row->token_id;
		// }
		...
		{
			$sqlQry="select * from tbl_user  where token_id='".$token."' ";
			$res=mysqli_query($this->dbconnection,$sqlQry);
			$row=mysqli_fetch_object($res);
		...
		{
			$sqlQry="select * from tbl_user  where token_id='".$token."' ";
			//$expiration = date('Y-m-d H:i:s', time() + (24 * 60 * 60)); // Expiration time is $expirationHours hours from now
			$res=mysqli_query($this->dbconnection,$sqlQry);
			$row=mysqli_fetch_object($res);
			@$token_date=$row->token_date;
			$new_time = date("Y-m-d H:i:s", strtotime('+24 hours'));
			@$expireddate=$token_date+$new_time;
			if($expireddate != ''){
			return $expireddate;
			}
		...
		function changePasswordToken($token)
		{
			
            $sqlselect="select * from tbl_user where token_id='".$token."' ";
			...
			if (mysqli_num_rows($resFound) != 0)
			{
				//$row=mysqli_fetch_object($resFound);
				$sql="Update tbl_user set
				password='".md5($this->confirmPassword)."', token_id='', token_date=''
